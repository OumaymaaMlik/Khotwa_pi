...
interface NavItem { label: string; icon: string; route: string; roles: UserRole[]; section?: string; }
...
export class LayoutComponent implements OnInit, OnDestroy {
  ...
  navItems: NavItem[] = [
    ...
    { label: 'Messages', icon: 'message', route: 'messages', roles: ['ADMIN'] },
  //  { label: 'Users', icon: 'users', route: 'utilisateurs', roles: ['ADMIN'] },
    ...
    { label: 'Subscriptions', icon: 'card', route: 'subscriptions', roles: ['ADMIN'], section: 'Finance' },
  ...
  getSectionIcon(section?: string): SafeHtml {
    if (section === 'Finance') return this.getIcon('card');
