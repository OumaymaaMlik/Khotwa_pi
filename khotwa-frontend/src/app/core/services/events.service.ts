...
updateStatus(idEvenement: number, statut: string): Observable<void> {
  ...
  return this.http.patch(`${this.apiUrl}/${id}/status?statut=${status}`, {});
}

generateActivityReport(): Observable<any> {
  // On suppose que l'ID du coach est récupéré par le backend via le token de session
  return this.http.post<any>(`${this.apiUrl}/generate-activity-report`, {})
    .pipe(catchError(this.handleError));
}
}