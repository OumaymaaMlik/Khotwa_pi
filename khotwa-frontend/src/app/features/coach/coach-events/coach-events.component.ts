...
export class CoachEventsComponent implements OnInit, OnDestroy {
  ...
  // ── Processing state (NEW) — button spinner per card ──
  processingId: number | null = null;

  // ── Activity Report (MÉTIER AVANCÉ) ──
  reportLoading = false;
  ...
  applyFiltersAndSort(): void {
    ...
    if (this.planFilter !== 'ALL') {
      result = result.filter(ev => ev.planType?.toUpperCase() === this.planFilter);
    }

    // 4. Type filter
    if (this.typeFilter !== 'ALL') {
      result = result.filter(ev => ev.type?.toUpperCase() === this.typeFilter);
    }
  ...
  // ════════════════════════════════════════
  // MÉTIER AVANCÉ — Activity Report
  // Calls backend to generate a full activity
  // report for this coach (PDF or email).
  // Wire to: eventService.generateActivityReport()
  // ════════════════════════════════════════
  ...
  checkNotifications(): void {
    ...
    if (this.pendingEvents.length > 0) {
      notifs.push({
        message: `${this.pendingEvents.length} event(s) awaiting your validation`,
        urgency: 'high'
      });
    }
    ...
    todayEvents.forEach(ev =>
      notifs.push({ message: `Reminder: "${ev.titre}" in less than 24h`, urgency: 'normal' })
    );
    ...
    if (weekEvents.length > 0) {
      notifs.push({
        message: `${weekEvents.length} accepted event(s) this week`,
        urgency: 'normal'
      });
    }
  ...
  prevMonth(): void {
    this.currentCalDate = new Date(
      ...
      this.currentCalDate.getMonth() - 1, 1
    );
    this.buildCalendar();
  }
