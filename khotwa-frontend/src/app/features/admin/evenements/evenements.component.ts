...
export class AdminEvenementsComponent implements OnInit {
  ...
  const creatorId = this.authService.currentUser?.idUser ?? 1; // doit être ADMIN

  const payload = {
      titre: this.newEvent.titre,
  description: this.newEvent.description,
  date: new Date(this.newEvent.date).toISOString().split('T')[0],                     // yyyy-MM-dd
  heure: this.formatHeure(this.newEvent.heure), // HH:mm:ss
  intervenant: this.newEvent.intervenant,
  lienMeet: this.newEvent.lienMeet,
  placesTotal: Number(this.newEvent.placesTotal),
  type: this.newEvent.type.toUpperCase(),       // FORMATION / PITCH / WEBINAR / COACHING
  ...
  confirmDelete(): void {
    ...
    this.eventService.delete(this.deleteTarget.idEvenement).subscribe({
      ...
      error: (err) => {
        // DELETE typically returns 204 No Content — Angular HttpClient
        // may throw a parse error on empty body, but deletion succeeds.
  ...
  private validateForm(form: Evenement, target: 'create' | 'edit'): boolean {
    ...
    if (!form.heure)               errors.heure       = 'Time is required';
    ...
    if (form.lienMeet && !/^https?:\/\/.+/.test(form.lienMeet)) {
      errors.lienMeet = 'Invalid URL (must start with http:// or https://)';
    }
