...
export class VisitorAnnoncesComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.talentService.getAnnonces().subscribe({
      ...
      error: () => { this.loadingList = false; this.error = 'Unable to load offers. Please try again later.'; },
    });
  }
  ...
  submitPostuler(): void {
    ...
    const tid = this.talentId;
    if (tid == null) {
      this.error = 'Please create your d’abord votre talent profile first (see My Talent Profile menu).';
      return;
    }
    ...
    this.talentService.postuler({
      ...
      next: (c) => {
        ...
        this.success =
          c.statut === 'ENREGISTRÉ_LOCALEMENT'
            ? 'Application saved locally (API not configured).'
      ...
      error: () => {
        this.submitting = false;
        this.error = 'Unable to save your application. Please try again.';
      },
    });
  }
