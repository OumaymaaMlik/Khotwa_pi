...
export class AuthService {
  ...
  refreshProfile(): Observable<UserResponse> {
    ...
    return this.http.get<UserResponse>(`${this.USERS_API}/me`, { headers }).pipe(
      tap(profile => {
        this._currentUser = {
          talentProfileId:     (profile as any).talentProfileId > 0 ? (profile as any).talentProfileId : undefined,
  ...
  private _saveLoginResponse(res: any): void {
    ...
    this._currentUser = {
      talentProfileId: res.talentProfileId > 0 ? res.talentProfileId : undefined,
      idUser:       res.idUser,
  ...
  private _saveProfileOnly(profile: any): void {
    this._currentUser = {
      talentProfileId: profile.talentProfileId > 0 ? profile.talentProfileId : undefined,
      idUser:       profile.idUser,
      id:           String(profile.idUser ?? ''),
      emailAddress: profile.emailAddress,
