...
public class MessageService {
    ...
    /**
     * FIX: Ce message système est envoyé par le système lors d'un initiateDirect.
     * Il ne doit PAS appeler ensureMember car le sender est garanti participant
     * (ensureDirectConversation l'a déjà ajouté). On passe directement par sendMessage.
     */
    public MessageDTO sendSystemMessageToConversation(Long conversationId, Long senderId, String content) {
        ...
        // FIX: receiver_id NOT NULL en BDD — déduire l'autre participant
        Long receiverId = conversationParticipantRepository
                .findByConversationIdAndActiveTrue(conversationId)
                .stream()
        ...
        // Utilise sendMessage directement (pas sendConversationMessage qui appelle ensureMember)
    ...
    public MessageDTO sendConversationMessage(Long conversationId, SendConversationMessageRequest request) {
        try {
            ...
            throw new IllegalArgumentException("user " + request.getSenderId() + " is not a member of conversation " + conversationId);
        }
        Conversation conversation = conversationRepository.findById(conversationId)
                .orElseThrow(() -> new RuntimeException("Conversation not found"));
        ...
        // FIX: receiver_id est NOT NULL en BDD (contrainte existante).
        // Pour les conversations DIRECT, on déduit le receiverId = l'autre participant.
        // Pour les conversations GROUP, on met le senderId lui-même comme fallback
        // (valeur non-null neutre) jusqu'à ce que la contrainte BDD soit retirée via migration.
        if (conversation.getType() == ConversationType.DIRECT) {
            Long receiverId = conversationParticipantRepository
                    .findByConversationIdAndActiveTrue(conversationId)
                    .stream()
                    .map(ConversationParticipant::getUserId)
                    .filter(uid -> !uid.equals(request.getSenderId()))
                    .findFirst()
                    .orElse(request.getSenderId()); // fallback: soi-même si seul dans la conv
            message.setReceiverId(receiverId);
        } else {
            // GROUP : pas de receiver unique — on met senderId pour satisfaire NOT NULL
            message.setReceiverId(request.getSenderId());
        }
